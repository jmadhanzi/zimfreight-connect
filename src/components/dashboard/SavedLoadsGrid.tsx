  ...
  if (items.length === 0) {
    return (
      <div className="rounded-md border border-dashed border-border bg-card/50 p-6 text-center text-xs text-muted-foreground">
        ...
        No saved loads. Bookmark loads from the <Link to="/board" search={(prev) => prev} className="text-primary hover:underline">Load Board</Link>.
      </div>
    );
  }
  return (
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      {items.map((l) => (
        <Link key={l.id} to="/board" search={(prev) => prev} className="group rounded-md border border-border bg-card p-3 transition-colors hover:border-primary">
