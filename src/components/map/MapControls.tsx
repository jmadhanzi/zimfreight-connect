      ...
      >
        ...
        <div className="flex items-center gap-2">
          <span className="font-mono text-[10px] font-bold tracking-tight text-muted-foreground">
            <span className="text-foreground">{filteredCount}</span>/{loadCount}
          </span>
          {expanded ? (
        ...
        <div className="space-y-4 border-t border-border px-4 pb-4 pt-3">
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-1.5">
              <select
                ...
                className="w-full rounded-lg border border-border bg-card px-3 py-2 text-xs font-medium text-foreground transition-colors hover:border-foreground/15 focus:outline-none focus:ring-2 focus:ring-secondary/40"
              >
            ...
            {(filterOrigin || filterDestination) && (
              <button
                ...
                className="font-mono text-[10px] font-bold uppercase tracking-[0.16em] text-secondary hover:underline"
              >
                Clear filters
              </button>
            )}
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-1.5 text-[11px] text-foreground/80">
              <LegendRow color="var(--primary)" label="Origin city (load count)" shape="circle" />
              <LegendRow color="var(--secondary)" label="Destination city" shape="circle" />
              <LegendRow color="var(--destructive)" label="Urgent load route" shape="line" />
              <LegendRow color="var(--info)" label="Cross-border route" shape="dashed" />
              <LegendRow color="var(--secondary)" label="Domestic route" shape="line" />
            </div>
            <div className="mt-3 space-y-1.5">
              ...
              {[
                ...
                <div key={label} className="flex items-center gap-2">
                  ...
                  />
                  <span className="text-[11px] text-foreground/75">{label}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="flex items-center gap-2">
        <span
          className={cn(
            ...
            active ? `${color} text-white` : "bg-muted text-muted-foreground",
          )}
        >
          {icon}
        </span>
...
function LegendRow({
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {shape === "line" && <div className="h-0.5 w-5 rounded-full" style={{ background: color }} />}
      {shape === "dashed" && (
        <div
          className="h-0.5 w-5"
          style={{
