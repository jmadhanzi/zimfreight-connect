...
import { Loader2, Lock, Truck, DollarSign, Route as RouteIcon, TrendingUp, Layers, MessageSquare, CheckCircle2, Clock } from "lucide-react";
...
import {
  useBorderStatus, useBrokerLoads, useBrokerStats, useCarrierBookings,
  useCarrierStats, useIncomingBids, useRouteRates, useSavedLoads, useSavedRoutes,
...
export const Route = createFileRoute("/dashboard")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Your loads, bookings, rates and notifications — personalized for carriers and brokers." },
    ],
  }),
  ...
  if (!user) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-20 text-center">
        ...
        <h1 className="mt-4 font-display text-3xl font-black uppercase tracking-tight">Sign in to view your dashboard</h1>
...
function CarrierSections() {
  ...
  const market = 2.84;
  const rateLabel = stats && stats.avg_rate_per_km > 0
    ? (stats.avg_rate_per_km >= market ? `vs $${market} · Above ▲` : `vs $${market} · Below ▼`)
    : `Market avg $${market}`;
